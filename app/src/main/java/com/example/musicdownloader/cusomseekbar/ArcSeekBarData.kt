...
internal data class ArcSeekBarData(
    ...
    val thumbX: Int = (r * cos(alphaRad + Math.PI / 2 - progressSweepRad).toFloat() + circleCenterX).toInt()
    val thumbY: Int = (-r * sin(alphaRad + Math.PI / 2 - progressSweepRad).toFloat() + circleCenterY).toInt()

    fun progressFromClick(x: Float, y: Float, thumbHeight: Int): Int? {
        if (y > height + dy * 2) return null
        val distToCircleCenter = sqrt((circleCenterX - x.toDouble()).pow(2.0) + (circleCenterY - y.toDouble()).pow(
            2.0
        )
        )
