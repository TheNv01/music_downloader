...
class GenericAdapter<T: Any, V: ViewDataBinding>(
    ...
    class GenericViewHolder(private var binding: ViewDataBinding) : RecyclerView.ViewHolder(binding.root) {
        fun <T: Any, V: ViewDataBinding> bind(item: T, bindingInterface: RecyclerBindingInterface<V, T>) =
           bindingInterface.binData(binding as V, item)
        }
    }


interface RecyclerBindingInterface<V: ViewDataBinding, T: Any>{
    fun binData(binder: V, model: T)
}